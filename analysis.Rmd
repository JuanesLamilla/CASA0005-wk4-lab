...
country_shp <- left_join(country_shp, global_gii_df[,c('iso3', 'gii_diff')], by = 'iso3')
```

### Mapping the Data

```{r mapping}
# Create a map of country_shp with gii_diff as the fill using a red/green gradient
ggplot(country_shp) +
  geom_sf(aes(fill = gii_diff)) +
  scale_fill_gradient2(low = 'darkgreen', mid = 'white', high = 'red', midpoint = 0) +
  theme_void() + 
  labs(title = "Global Gender Inequality Index difference between 1990 and 2019")
